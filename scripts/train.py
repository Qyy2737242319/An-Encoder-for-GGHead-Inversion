...
def train(opts,
    ...
    # loss_vgg = VGGLoss(device=device)


    random_index = [index for index in range(100)]
    ...
    for i in tqdm.tqdm(range(opts.resume_step,opts.iteration)):
        with torch.no_grad():

            index = np.random.choice(random_index, 1)[0].astype(int)
            ...
            for j in range(1,batch_size):
                ...
                #image = np.concatenate((image,np.expand_dims(np.array(Image.open(f"./dataset/{int(index[j])}.jpg")),axis=0)),axis=0)
        ...
        if i<opts.stop_encoder:
            ...
            #test = pre_image
            #test = Img.from_normalized_torch(output['image'][:, :3, ...]).to_torch().img
            # test_img = test * 255
            # test_img = test_img[0,...].permute(1,2,0).detach().cpu().numpy()
            # save_img = Image.fromarray(test_img.astype(np.uint8))
            # save_img.save("/home/yang/gghead/finetuning/test.png")
            loss_psp, loss_dict, id_logs = calc_loss_psp(test_image.clone().detach(),pre_image, loss_dict,opts,Id_loss, lpips_loss)
            # mse_loss = 20*loss_fn(pre_image, true_image)
            # # l1_loss = loss_l1(rec_ws,z)
            # vgg_loss = 0.2*loss_vgg(pre_image, true_image)

            #loss = mse_loss + vgg_loss # + 0.1*l1_loss
            # loss += loss_fn_lpips(test_image, pre_image)
            optimizer_encoder.zero_grad()
            loss_psp.backward()
        ...
        if i != 0 and i%1000==0:
            ...
            with torch.no_grad():
                ...
                if i > opts.triplane:
                    ...
                    output_folder = f"./finetuning/test_result/"
                    ensure_directory_exists(output_folder)
                    mediapy.write_video(f"{output_folder}/step_{i}_offsets.mp4", all_frames, fps=24)

        if i!=0 and  i % 5000==0:
            torch.save(Encoder.state_dict(), f'./ckpt/E_params_{i}.pth')
            optimizer_state = optimizer_encoder.state_dict()
            torch.save(optimizer_state, f'./ckpt/optimizer_psp_{i}.pth')
            torch.save(triplane_encoder.state_dict(), f'./ckpt/T_params_{i}.pth')
            torch.save(optimizer_triplane.state_dict(), f'./ckpt/optimizer_triplane_{i}.pth')
