...
def train(opts,
    ...
    E_psp = psp_Encoder(50, 'ir_se', opts).to(device)
    E_psp.load_state_dict(torch.load("./inversion_E_params_36000.pth", map_location=device))
    #E.load_state_dict(torch.load("./E_params_32000.pth", map_location=device))
    triplanenet_encoder = tri_Encoder(opts).to(device)
    triplanenet_encoder.load_state_dict(torch.load("./inversion_T_params_36000.pth", map_location=device))

    test_img = np.array(Image.open('/home/yang/gghead/dataset/150.jpg').convert('RGB'))
    ...
    for i in tqdm.tqdm(range(512)):
        ...
        edit_w[0, i] = sigma * rec_ws[0, i]  #3hair color 7 smile   501 sex
        #50+- biaoqing
        #150+- light guangzhao
        #199 hair style
        ...
        with torch.no_grad():
            ...
            output_folder = f"/home/yang/gghead/finetuning/test_result/"
            ensure_directory_exists(output_folder)
            mediapy.write_video(f"{output_folder}/inversion.mp4", all_frames, fps=24)

            pre_image = output_test['image'][:, :3, ...]
            resize = transforms.Resize((256, 256))
            #codes = rec_ws.clone().detach()
            ...
            # save_img.save("/home/yang/gghead/test.png")
            ...
            triplane_offsets = triplanenet_encoder(x_input, output_test["triplanes"].clone().detach())
            ...
            for c_batch, w_batch in zip(batchify_sliced(c_front, batch_size=batch_size),
                ...
                #all_frames.extend(frames)
            output_folder = f"/home/yang/gghead/finetuning/5/"
            ensure_directory_exists(output_folder)
            # #mediapy.write_video(f"{output_folder}/inversion_offset.mp4", all_frames, fps=24)
            # test = Img.from_normalized_torch(outs['image'][:, :3, ...]).to_torch().img
            # test_img = test * 255
            # test_img = test_img[0, ...].permute(1, 2, 0).cpu().numpy()
            save_img = Image.fromarray(frames[0].astype(np.uint8))
            save_img.save(os.path.join(output_folder,f"{i}.png"))
